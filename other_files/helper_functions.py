    ...
    if os.path.isfile(img_file) and os.access(img_file, os.R_OK):
        try:
            ...
            # if the original file is a scanned image pdf, makes a new PDF/A file, with a text layer that can be extracted.
                ...
                ocrmypdf.ocr(img_file, pdfa_file, deskew=True, progress_bar=False, language = 'por')
...
def prep_email_data(txt_ext):
    ...
    subj_re = re.compile(r'(PFIs|PCOs) Aprovad(a|o)s em (\d\d-\d\d-\d\d\d\d) (<> )?Lote(:)? (\d{5})( <>)? ')
    ...
    if not subject is None:
        ...
        email_full_file_name = 'RE DSL email receiving ' + pfi_pco + ' lote ' + lote_nr  + ' date ' + date_transm
...
def get_comp_filename(server, doc_type, nr, pco_year, dest_file_name):
    ...
    path_to_folder = server+'\\PCOs\\ORDERS ' + pco_year + '\\' + pco + nr + '\\' + sub_folder
    path_to_file = server+'\\PCOs\\ORDERS ' + pco_year + '\\' + pco + nr + '\\' + sub_folder + '\\' + dest_file_name + '.pdf'
...
def copy_files_to_folders(ocr_file, path_to_folder, path_to_file):
    ...
    if os.path.exists(path_to_folder):
        ...
        if os.path.exists(path_to_file):
            ...
            path_to_file = os.path.splitext(path_to_file)[0] + '(1)' + os.path.splitext(path_to_file)[1]
        ...
        print(f'The original file: {os.path.basename(path_to_file)} was duly archived.')

    else:
        # Inform user the folder extracted from the file is not valid for archive.
        print('The folder extracted from the file,\n{os.path.basename(path_to_folder)}\nis not valid for archive.')
        
        # Log the folder extracted from the file is not valid for archive.
        logging.info(f'The folder extracted from the file {os.path.basename(path_to_folder)} is not valid for arquive.')
